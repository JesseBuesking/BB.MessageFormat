...
app.post('/mf', function (req, res) {
    var c_lookup = compile_lookup[req.body.l];
    var message = undefined;
    if (c_lookup) {
        message = c_lookup[req.body.c];
    }
    if (!message) {
        mf = lookup[req.body.l];
        if (!mf) {
            mf = lookup[req.body.l] = new MessageFormat(req.body.l);
        }
        message = mf.compile(req.body.c);
        if (!compile_lookup[req.body.l]) {
            compile_lookup[req.body.l] = [];
        }
        compile_lookup[req.body.l][req.body.c] = message;
    }
    res.send(message(req.body.m));
})
.listen(1234);
